...
if not args.skip_training:
    ...
    if args.mode == "big":
        ...
        for scene in mipnerf360_outdoor_scenes:
            ...
            CMD = "python train_faster.py -s " + source + " -i images_4 -m " + args.output_path + "/" + f"{scene}_big" + common_args + budget_param + mode_param
            run_cmd(CMD, args)
        for scene in mipnerf360_indoor_scenes:
            source = args.mipnerf360 + "/" + scene
            budget_param = " --budget {} ".format(big_budgets[scene])
            CMD = "python train_faster.py -s " + source + " -i images_2 -m " + args.output_path + "/" + f"{scene}_big" + common_args + budget_param + mode_param
            run_cmd(CMD, args)
        ...
        for scene in tanks_and_temples_scenes:
            ...
            budget_param = " --budget {} ".format(big_budgets[scene])
            CMD = "python train_faster.py -s " + source + " -m " + args.output_path + "/" + f"{scene}_big" + common_args + budget_param + mode_param
            run_cmd(CMD, args)
        ...
        for scene in mipnerf360_outdoor_scenes:
            ...
            CMD = "python train_faster.py -s " + source + " -i images_4 -m " + args.output_path + "/" + f"{scene}_budget" + common_args + budget_param + mode_param
            run_cmd(CMD, args)
        for scene in mipnerf360_indoor_scenes:
            source = args.mipnerf360 + "/" + scene
            budget_param = " --budget {} ".format(budget_multipliers[scene])
            CMD = "python train_faster.py -s " + source + " -i images_2 -m " + args.output_path + "/" + f"{scene}_budget" + common_args + budget_param + mode_param
            run_cmd(CMD, args)
        m360_timing = (time.time() - start_time)/60.0
        ...
        for scene in tanks_and_temples_scenes:
            source = args.tanksandtemples + "/" + scene
            budget_param = " --budget {} ".format(budget_multipliers[scene])
            CMD = "python train_faster.py -s " + source + " -m " + args.output_path + "/" + f"{scene}_budget" + common_args + budget_param + mode_param
            run_cmd(CMD, args)
        tandt_timing = (time.time() - start_time)/60.0
...
if not args.skip_rendering:
    if args.mode == "big":
        for scene in all_scenes:
            ...
            CMD = f"python render_faster.py -m {output_path} --skip_train"
            run_cmd(CMD, args)
