...
from gaussian_renderer.render_faster import render_faster
import sys
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, websockets, mult_res):
    first_iter = 0
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        if websockets:
            if network_gui_ws.curr_id >= 0 and network_gui_ws.curr_id < len(scene.getTrainCameras()):
                ...
                net_image = render_faster(cam, gaussians, pipe, background, opt.mult, 1.0)["render"]
        ...
        render_pkg = render_faster(viewpoint_cam, gaussians, pipe, bg, opt.mult, render_size=gt_image.shape[-2:], is_training=True)
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs):
    if tb_writer:
        ...
        tb_writer.add_scalar('xyz_lr', scene.gaussians.update_learning_rate(iteration), iteration)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mult_res", action="store_true")
