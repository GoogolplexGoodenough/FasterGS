...
from gaussian_renderer.render_mkgs_ms import render_imp_mkgs as render_MKGS
...
def compute_gaussian_score_fastgs(camlist, gaussians, pipe, bg, args, DENSIFY = False, render_scale = 1.0):
    ...
    for view in range(len(camlist)):
        ...
        render_image = render_MKGS(my_viewpoint_cam, gaussians, pipe, bg, mult=0.5)["render"]
        photometric_loss = compute_photometric_loss(my_viewpoint_cam, render_image)

        get_flag = True
        l1_loss_norm = get_loss(render_image, gt_image)
        
        metric_map = (l1_loss_norm > 0.1).int()

        render_pkg = render_MKGS(my_viewpoint_cam, gaussians, pipe, bg, mult=0.5, get_flag = get_flag, metric_map = metric_map)
