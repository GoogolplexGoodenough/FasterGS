...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, culling=None):
    """
    Render the scene. 
    ...
    from MKGS_ms import GaussianRasterizationSettings, GaussianRasterizer
...
def render_imp_mkgs(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, grad_info=False, culling=None, get_flag=False, metric_map=None, mult=0.5):
    """
    Render the scene. 
    ...
    # Create zero tensor. We will use it to make pytorch return gradients of the 2D (screen-space) means
    from MKGS import GaussianRasterizationSettings, GaussianRasterizer
    ...
    raster_settings = GaussianRasterizationSettings(
        ...
        mult=mult,
        get_flag=get_flag,
        metric_map=torch.tensor([], dtype=torch.int, device='cuda') if not get_flag else metric_map
    )
