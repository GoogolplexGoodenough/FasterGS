...
from gaussian_renderer.render_faster import render_imp
import sys
...
try:
    from diff_gaussian_rasterization_dash import SparseGaussianAdam
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        while network_gui.conn != None:
            try:
                ...
                if custom_cam != None:
                    net_image = render_imp(custom_cam, gaussians, pipe, background, scaling_modifier=scaling_modifer, use_trained_exp=dataset.train_test_exp, separate_sh=SPARSE_ADAM_AVAILABLE)["render"]
        ...
        render_pkg = render_imp(viewpoint_cam, gaussians, pipe, bg, render_size=gt_image.shape[-2:], xyz_grad=True)
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                progress_bar.set_postfix({"Loss": f"{ema_loss_for_log:.{7}f}", "N_GS": f"{gaussians.get_xyz.shape[0]}", "N_MAX": f"{scheduler.max_n_gaussian}", "R": f"{render_scale}"})
