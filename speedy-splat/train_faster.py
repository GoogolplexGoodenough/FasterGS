...
def training(dataset, opt, pipe, testing_iterations, visualize_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if (iteration - 1) == debug_from:
            pipe.debug = True
        render_pkg = render_imp(viewpoint_cam, gaussians, pipe, background, xyz_grad=True)
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                progress_bar.set_postfix({"Loss": f"{ema_loss_for_log:.{7}f}", "#GS": f"{gaussians.get_xyz.shape[0]}"})
