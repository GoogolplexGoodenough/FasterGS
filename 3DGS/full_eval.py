...
if not args.skip_training:
    ...
    for scene in mipnerf360_outdoor_scenes:
        ...
        if args.fastergs:
            os.system("python train_faster.py -s " + source + " -i images_4 -m " + args.output_path + "/" + scene + common_args)
        else:
            os.system("python train.py -s " + source + " -i images_4 -m " + args.output_path + "/" + scene + common_args)
    for scene in mipnerf360_indoor_scenes:
        source = args.mipnerf360 + "/" + scene
        if args.fastergs:
            os.system("python train_faster.py -s " + source + " -i images_2 -m " + args.output_path + "/" + scene + common_args)
        else:
    ...
    for scene in tanks_and_temples_scenes:
        ...
        if args.fastergs:
            os.system("python train_faster.py -s " + source + " -m " + args.output_path + "/" + scene + common_args)
        else:
...
if not args.skip_rendering:
    ...
    for scene, source in zip(all_scenes, all_sources):
        if args.fastergs:
            os.system("python render_faster.py --iteration 30000 -s " + source + " -m " + args.output_path + "/" + scene + common_args)
        else:
